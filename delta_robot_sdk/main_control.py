...
import re
from utils import extract_value, format_goto_command, format_gcode_buffer
...
class MainControlTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ===== G-CODE BUFFER MODE VARIABLES =====
        self.gcode_buffer_mode = False
        self.gcode_points = []  # Store points for continuous motion
        self.buffer_mode_active = False
    ...
    def set_gcode_buffer_mode(self, enabled):
        """Enable/disable G-code buffer mode"""
        self.gcode_buffer_mode = enabled
        if enabled:
            self.buffer_mode_btn.setText("📦 Buffer Mode ON")
            self.buffer_mode_btn.setStyleSheet("background-color: #ff9800; color: white;")
            self.log_message.emit("INFO", "G-code buffer mode activated")
        else:
            self.buffer_mode_btn.setText("📦 Buffer Mode OFF")
            self.buffer_mode_btn.setStyleSheet("")
            self.log_message.emit("INFO", "Normal mode activated")
            # Clear buffer if switching off
            self.gcode_points.clear()
    
    def toggle_buffer_mode(self, checked):
        """Toggle buffer mode on/off"""
        self.set_gcode_buffer_mode(checked)
    
    def add_to_buffer(self, x, y, z):
        """Add point to G-code buffer"""
        self.gcode_points.append((x, y, z))
        self.log_message.emit("INFO", f"Buffer: {len(self.gcode_points)} points")
        
        # Auto-send when buffer reaches 20 points (Arduino buffer size)
        if len(self.gcode_points) >= 20:
            self.send_buffer()
    
    def clear_buffer(self):
        """Clear the G-code buffer"""
        self.gcode_points.clear()
        self.log_message.emit("INFO", "Buffer cleared")
    
    def send_buffer(self):
        """Send all buffered points as one command"""
        if len(self.gcode_points) == 0:
            return
        
        # Format points for Arduino planner
        buffer_cmd = format_gcode_buffer(self.gcode_points)
        self.program_command.emit(buffer_cmd)
        self.log_message.emit("INFO", f"Sent {len(self.gcode_points)} points to planner")
    ...
    def create_left_panel(self):
        ...
        # Buffer mode toggle button
        self.buffer_mode_btn = QPushButton("📦 Buffer Mode OFF")
        self.buffer_mode_btn.setCheckable(True)
        self.buffer_mode_btn.setMinimumHeight(30)
        self.buffer_mode_btn.clicked.connect(self.toggle_buffer_mode)
        layout.addWidget(self.buffer_mode_btn)
        
        # Clear buffer button
        clear_buffer_btn = QPushButton("🗑️ Clear Buffer")
        clear_buffer_btn.setMinimumHeight(30)
        clear_buffer_btn.clicked.connect(self.clear_buffer)
    ...
    def create_right_panel(self):
        """Create right panel with Position Control"""
    ...
    # ===== MODIFIED: play_program with buffer mode support =====
    def play_program(self):
        """Play program - with buffer mode support"""
        ...
        # If in buffer mode and we have points, send them directly
        if self.gcode_buffer_mode and len(self.gcode_points) > 0:
            self.send_buffer()
            return
    ...
    # ===== MODIFIED: execute_current_line with G-code buffer support =====
    def execute_current_line(self):
        """Execute current line - with G-code buffer support"""
        if self.current_line < self.program_list.count():
            ...
            # Check if buffer mode is enabled AND this is a G01/G00 command
            if self.gcode_buffer_mode and ("G00" in command or "G01" in command):
                try:
                    x = extract_value(command, 'X')
                    y = extract_value(command, 'Y')
                    z = extract_value(command, 'Z')
                    ...
                    if x and y and z:
                        self.add_to_buffer(float(x), float(y), float(z))
                        self.buffer_mode_active = True
                    else:
                        # If we can't extract coordinates, send normally
            ...
            # If not in buffer mode or not a movement command, send normally
            else:
                self.program_command.emit(command)
                self.log_message.emit("INFO", f"Executing: {command}")
            
            # Schedule next line if program is running
            if self.program_running and not self.program_paused:
                if self.current_line < self.program_list.count() - 1:
                    QTimer.singleShot(45, self.execute_next_line)
                else:
                    # Program complete - send any remaining buffered points
                    if len(self.gcode_points) > 0 and self.buffer_mode_active:
                        self.send_buffer()
                    self.program_complete()
    ...
    def program_complete(self):
        ...
        # Reset buffer mode
        if self.buffer_mode_active:
            self.buffer_mode_active = False
            if self.gcode_buffer_mode:
                self.buffer_mode_btn.setText("📦 Buffer Mode ON")
        
        self.log_message.emit("INFO", "Program complete")
