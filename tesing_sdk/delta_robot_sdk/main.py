...
from utils import format_goto_command, get_timestamp
...
class DeltaRobotGUI(QMainWindow):
    ...
    def on_connection_status(self, connected, message):
        ...
        if connected:
            ...
            # Send test command
            QTimer.singleShot(50, lambda: self.serial_thread.send_command("TEST"))
        else:
    ...
    def execute_program_command(self, command):
        ...
        if "G01" in command or "G00" in command:
            ...
            if x and y and z:
                try:
                    time_val = self.main_control.time_spin.value()
    ...
    # main.py mein ye changes karo

    # Top par import karo
   

    # setup_central_widget() method mein tabs mein add karo:
    ...
    def setup_central_widget(self):
        ...
        self.main_control = MainControlTab(self)
        self.config_tab = ConfigTab(self)
        self.vision_tab = VisionTab(self)  # ← YEH ADD KARO
        self.log_tab = LogTab(self)
        ...
        self.vision_tab.log_message.connect(self.add_log)  # ← YEH ADD KARO
        
        # Add tabs
        self.tab_widget.addTab(self.main_control, "🎮 MAIN CONTROL")
        self.tab_widget.addTab(self.config_tab, "⚙️ CONFIG")
        self.tab_widget.addTab(self.vision_tab, "📷 VISION")  # ← YEH ADD KARO
        self.tab_widget.addTab(self.log_tab, "📋 SYSTEM LOG")
